...
class MolecularComplexMetadata:
    ...
    # (JH) fixed: retain the entity assigned to each model chain so CIF
    # serialization does not have to infer identity from residue names.
    chain_to_entity: dict[int, int] | None = None
...
class MolecularComplex:
    ...
    def from_mmcif(cls, inp: str, id: str | None = None) -> "MolecularComplex":
        ...
        atom_site_entity_id_by_chain: dict[str, int] = {}
            ...
            if "label_asym_id" in atom_site:
                ...
                if len(filtered) == len(structure):
                    ...
                    if "label_entity_id" in atom_site:
                        _entity_col = atom_site["label_entity_id"]
                        _raw_entity_ids = (
                            _entity_col.as_array(str)
                            if hasattr(_entity_col, "as_array")
                            else np.array(list(_entity_col), dtype=str)
                        )
                        filtered_entity_ids = _raw_entity_ids[keep]
                        atom_site_entity_id_by_chain = {
                            str(chain_id): int(entity_id)
                            for chain_id, entity_id in zip(
                                filtered, filtered_entity_ids, strict=True
                            )
        ...
        try:
            if "entity" in block:
                ...
                if "id" in entity_category and "type" in entity_category:
                    entity_ids = entity_category["id"].as_array(str)
                    entity_types = entity_category["type"].as_array(str)
                    # (JH) fixed: CIFColumn is not guaranteed to be directly
                    # iterable; preserve declared polymer/non-polymer types.
                    entity_info = {
                        int(entity_id): str(entity_type)
                        for entity_id, entity_type in zip(
                            entity_ids, entity_types, strict=True
                        )
                    }
        except Exception:
            pass
        entity_id_by_chain: dict[str, int] = {}
        try:
            if "struct_asym" in block:
                struct_asym = block["struct_asym"]
                chain_ids = struct_asym["id"].as_array(str)
                entity_ids = struct_asym["entity_id"].as_array(str)
                entity_id_by_chain = {
                    str(chain_id): int(entity_id)
                    for chain_id, entity_id in zip(chain_ids, entity_ids, strict=True)
                }
        except Exception:
            pass
        if not entity_id_by_chain:
            # (JH) fixed: retain declared molecule identity from atom_site when
            # a valid CIF omits the optional struct_asym category.
            entity_id_by_chain = atom_site_entity_id_by_chain
        ...
        chain_to_entity: dict[int, int] | None = {
            numeric_id: entity_id_by_chain[chain_id]
            for chain_id, numeric_id in chain_id_to_numeric.items()
            if chain_id in entity_id_by_chain
        }
        if len(chain_to_entity) != len(chain_lookup):
            chain_to_entity = None
    ...
    def _get_entity_mapping(
        ...
        numeric_chain_id_by_label: dict[str, int] = {}
        for token_idx in range(len(self.token_to_atoms)):
            chain_id_numeric = int(self.chain_id[token_idx])
            chain_id_str = self.metadata.chain_lookup.get(
                chain_id_numeric, chr(65 + chain_id_numeric)
            )
            numeric_chain_id_by_label[chain_id_str] = chain_id_numeric
        ...
        if self.metadata.chain_to_entity is not None:
            declared_chain_to_entity = {
                int(chain_id): int(entity_id)
                for chain_id, entity_id in self.metadata.chain_to_entity.items()
            }
            observed_chain_ids = {
                int(chain_id) for chain_id in np.unique(self.chain_id)
            }
            if not observed_chain_ids.issubset(declared_chain_to_entity):
                raise ValueError(
                    "MolecularComplex chain_to_entity does not cover every "
                    "observed chain"
                )
            ...
            for chain_id_str, sequence in chain_sequences.items():
                numeric_chain_id = numeric_chain_id_by_label[chain_id_str]
                entity_id = declared_chain_to_entity[numeric_chain_id]
                sequence_tuple = tuple(sequence)
                previous_sequence = entity_sequences.setdefault(
                    entity_id, sequence_tuple
                )
                if previous_sequence != sequence_tuple:
                    raise ValueError(
                        f"MolecularComplex entity {entity_id} has inconsistent "
                        "chain sequences"
                    )
                chain_to_entity[chain_id_str] = entity_id
    ...
    def _get_entity_types(
        self, entity_sequences: dict[int, tuple[str, ...]]
    ) -> dict[int, str]:
        entity_types: dict[int, str] = {}
        for entity_id, sequence in entity_sequences.items():
            declared_type = self.metadata.entity_lookup.get(entity_id)
            normalized_type = (
                str(declared_type).strip().lower()
                if declared_type is not None
                else None
            )
            if normalized_type in {"polymer", "non-polymer"}:
                entity_types[entity_id] = normalized_type
                continue

            has_protein = any(
                token in residue_constants.restype_3to1 for token in sequence
            )
            has_nucleic_acid = any(
                token in ("A", "T", "G", "C", "U", "DA", "DT", "DG", "DC")
                for token in sequence
            )
            entity_types[entity_id] = (
                "polymer" if has_protein or has_nucleic_acid else "non-polymer"
            )
        return entity_types
    ...
    def _add_entity_information(
        ...
        for eid in sorted(entity_sequences.keys()):
            ...
            entity_type = entity_type_by_id[eid]
            entity_types.append(entity_type)
            if entity_type == "polymer":
                if has_na and not has_protein:
        ...
        for eid in sorted(entity_sequences.keys()):
            ...
            if entity_type_by_id[eid] != "polymer":
                continue  # non-polymer entity, OST doesn't need SEQRES

            if has_protein or not has_na:
    ...
    def to_mmcif(self) -> str:
        ...
        # Build entity mappings: model outputs retain their declared entities;
        # older MolecularComplex objects fall back to sequence identity.
        _, chain_to_entity, entity_sequences = self._get_entity_mapping()
        entity_type_by_id = self._get_entity_types(entity_sequences)
        ...
        for token_idx, (start, end) in enumerate(self.token_to_atoms):
            ...
            entity_id = chain_to_entity.get(chain_id_str, 1)
            is_polymer = entity_type_by_id.get(entity_id) == "polymer"
            is_nucleic_acid = token in ("A", "T", "G", "C", "U", "DA", "DT", "DG", "DC")
            is_protein = is_polymer and not is_nucleic_acid
        ...
        # (JH) fixed: write the model-declared molecule type instead of
        # reclassifying amino-acid CCD ligands such as GLU or MET as polymers.
        self._add_entity_information(cif_file, entity_sequences, entity_type_by_id)
