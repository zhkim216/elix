...
from allatom_design.eval.metrics.protein_correspondence import ca_by_pn_unit_res_id
...
def _joint_resolved_reference_sample_pred_ca_indices(
    *,
    ...
    try:
        reference_ca_by_key, _ = ca_by_pn_unit_res_id(
            reference_atom_array, receptor_pn_unit_iids
        )
        sample_ca_by_key, _ = ca_by_pn_unit_res_id(
            sample_atom_array, receptor_pn_unit_iids
        )
        pred_ca_by_key, _ = ca_by_pn_unit_res_id(
            pred_atom_array, receptor_pn_unit_iids
        )
    ...
    if len(common_keys) == 0:
        return None, None, None, "No common receptor CA exact residue IDs found"
    ...
    reference_ca_resolved = (
        ~np.isnan(reference_atom_array[reference_ca_indices].coord).any(axis=1)
        & (reference_atom_array.res_name[reference_ca_indices] != "UNK")
    )
    sample_ca_resolved = (
        ~np.isnan(sample_atom_array[sample_ca_indices].coord).any(axis=1)
        & (sample_atom_array.res_name[sample_ca_indices] != "UNK")
    )
    pred_ca_resolved = (
        ~np.isnan(pred_atom_array[pred_ca_indices].coord).any(axis=1)
        & (pred_atom_array.res_name[pred_ca_indices] != "UNK")
    )
...
def _compute_small_molecule_docking_metrics_atomarray(
    *,
    ...
    # Match receptor CAs by exact label residue IDs across reference/design/prediction.
    reference_ca_indices, sample_ca_indices, pred_ca_indices, error = _joint_resolved_reference_sample_pred_ca_indices(
        reference_atom_array=reference_atom_array,
        sample_atom_array=sample_atom_array,
        pred_atom_array=pred_atom_array,
