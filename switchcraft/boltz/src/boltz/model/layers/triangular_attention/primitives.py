...
class Attention(nn.Module):
    ...
    def forward(
        ...
        q, k, v = self._prep_qkv(q_x, kv_x, apply_scale=not (use_deepspeed_evo_attention or use_cueq))
        ...
        #     assert biases is not None and len(biases) == 2
        #     mask_bias, tri_bias = biases

        #     mask_ce = mask_bias == 0 
        #     o = cueq.triangle_attention(
        #         q, k, v,
        #         tri_bias,
        #         mask=mask_ce,
        #         scale=1.0,      # dont forget cuz prep_qkv alr scales
        #     )
...
def _cueq_triangle_attn(q, k, v, biases, scale):
    ...
    o = cueq.triangle_attention(q, k, v, bias=triangle_bias, mask=mask_bias, scale=scale)
