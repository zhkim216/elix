...
class Protein:
    ...
    # optional ligand parsing
    ligand_positions: Optional[np.ndarray] = None        # [num_ligand_atoms, 3]
    ligand_chain_index: Optional[np.ndarray] = None      # [num_ligand_atoms]
    ligand_atomnames: Optional[Sequence[str]] = None      # [num_ligand_atoms]
...
def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None) -> Protein:
    ...
    for chain in model:
        for res in chain:
            hetflag, resseq, icode = res.id
            if not hetflag.strip():  # only hetero residues (ligands, metals)
                continue
            if res.resname in ("HOH", "WAT"):  # skip waters
                continue
            for atom in res:
                ligand_positions.append(atom.coord)
                ligand_chain_index.append(chain.id)
                ligand_atom_names.append(atom.name.strip())


    chain_id_mapping = {cid: n for n, cid in enumerate(string.ascii_uppercase)}
    chain_index = np.array([chain_id_mapping[cid] for cid in chain_ids])
    segment_index = np.array([chain_id_mapping.get(cid, -1) for cid in segment_ids])

    if len(ligand_positions) > 0:
        ligand_positions = np.array(ligand_positions)
        ligand_chain_index = np.array([chain_id_mapping.get(cid, -1) for cid in ligand_chain_index])
    else:
        ligand_positions = None
        ligand_chain_index = None
        ...
        for l in pdb_str.split("\n"):
            if "PARENT" in l:
                if "N/A" not in l:
                    ...
                    parents_chain_index.extend([chain_counter for _ in parent_names])
                chain_counter += 1
    ...
    return Protein(
        ...
        ligand_chain_index=ligand_chain_index,
        ligand_atomnames=ligand_atom_names,
    )
