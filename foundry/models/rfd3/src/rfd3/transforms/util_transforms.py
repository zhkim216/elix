...
# (JH) added: AtomWorks chain_type is authoritative for explicit non-polymers.
# Keep the residue-name fallback because de novo residues do not have chain_type
# until the assembled AtomArray is reparsed before the main model pipeline.
def _classify_token_molecule_types(
    atom_array: AtomArray,
    sequence_encoding: AF3SequenceEncoding = sequence_encoding,
) -> dict[str, np.ndarray]:
    ...
    is_protein = np.isin(res_names, aa_like_res_names).astype(bool)
    is_rna = np.isin(res_names, rna_like_res_names).astype(bool)
    is_dna = np.isin(res_names, dna_like_res_names).astype(bool)

    if "chain_type" in atom_array.get_annotation_categories():
        is_non_polymer = (
            np.asarray(atom_array.chain_type[token_starts])
            == ChainType.NON_POLYMER.value
        )
        is_protein &= ~is_non_polymer
        is_rna &= ~is_non_polymer
        is_dna &= ~is_non_polymer

    is_ligand = ~(is_protein | is_rna | is_dna)
    is_residue = np.isin(res_names, STANDARD_AA).astype(bool) & is_protein
...
def assign_types_(atom_array):
    ...
    molecule_types = _classify_token_molecule_types(atom_array)

    # Set annotations
    for annotation, token_values in molecule_types.items():
        atom_array.set_annotation(annotation, token_values[atom_to_token_map])
...
class EncodeAF3TokenLevelFeatures(Transform):
    ...
    def forward(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # (JH) added: Reuse the same NON_POLYMER-aware classification used by
        # AssignTypes instead of reclassifying model features from CCD names.
        molecule_types = _classify_token_molecule_types(
            atom_array, sequence_encoding=self.sequence_encoding
        )
        is_protein = molecule_types["is_protein"]
        is_rna = molecule_types["is_rna"]
        is_dna = molecule_types["is_dna"]
        is_ligand = molecule_types["is_ligand"]
        ...
        # ... sequence tokens
        res_names = token_level_array.res_name.copy()
        ...
        # (JH) added: Ligand chemistry comes from atom/reference features; an
        # amino-acid-like CCD name must not leak a protein restype to the model.
        res_names[is_ligand] = "UNK"
