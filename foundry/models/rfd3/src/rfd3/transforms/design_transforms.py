...
class AddGroundTruthSequence(Transform):
    ...
    def forward(self, data: dict) -> dict:
        atom_array = data["atom_array"]
        token_starts = get_token_starts(atom_array)
        res_names = atom_array.res_name[token_starts].copy()

        # (JH) added: NON_POLYMER CCDs such as MET/GLU are ligand chemistry,
        # not valid protein sequence targets even though their names are amino acids.
        is_ligand = atom_array.is_ligand[token_starts].astype(bool)
        ...
        data["ground_truth"]["sequence_valid_mask"] = torch.from_numpy(
            ~np.isin(res_names, ["UNK", "X", "DX", "<G>"]) & ~is_ligand
        )
