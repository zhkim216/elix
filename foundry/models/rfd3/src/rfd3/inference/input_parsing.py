...
# (JH) added: Preserve explicit non-polymer identity across AtomArray transports.
def preserve_explicit_nonpolymer_identity(atom_array_orig):
    """Adapt explicit non-polymer identity for AtomArray consumers.

    Some AtomArray consumers infer chain identity from the ``hetero``
    annotation rather than an authoritative CIF entity table.  That heuristic
    is ambiguous for a standard amino-acid CCD used as a standalone ligand:
    its CCD template may have ``hetero=False`` even though the input explicitly
    annotates the chain as non-polymeric.

    Work on a copy and normalize only the observed inconsistent transport
    state: ``chain_type=NON_POLYMER``, ``is_polymer=False``, and
    ``hetero=False``.  Never clear an existing HETATM flag, which preserves
    modified residues in polymer chains.
    """
    atom_array = copy.deepcopy(atom_array_orig)
    annotations = set(atom_array.get_annotation_categories())
    if "chain_type" not in annotations or "is_polymer" not in annotations:
        return atom_array

    try:
        chain_types = [ChainType.as_enum(value) for value in atom_array.chain_type]
    except ValueError as exc:
        raise ValueError(
            "Cannot preserve explicit chain identity: invalid `chain_type` annotation."
        ) from exc

    is_explicit_non_polymer = np.asarray(
        [chain_type == ChainType.NON_POLYMER for chain_type in chain_types],
        dtype=bool,
    )
    normalize_mask = (
        is_explicit_non_polymer
        & ~np.asarray(atom_array.is_polymer, dtype=bool)
        & ~np.asarray(atom_array.hetero, dtype=bool)
    )
    if np.any(normalize_mask):
        identities = sorted(
            {
                f"{chain_id}{int(res_id)}:{res_name}"
                for chain_id, res_id, res_name in zip(
                    atom_array.chain_id[normalize_mask],
                    atom_array.res_id[normalize_mask],
                    atom_array.res_name[normalize_mask],
                    strict=True,
                )
            }
        )
        atom_array.hetero[normalize_mask] = True
        logger.warning(
            "Preserving explicit non-polymer identity across AtomArray transport; "
            f"normalized hetero=True for {len(identities)} residue(s): {identities[:8]}"
        )
...
def prepare_pipeline_input_from_atom_array(  # see atomworks.ml.datasets.parsers.base.load_example_from_metadata_row
    ...
    # (JH) added: Parse an adapted copy without mutating the caller's AtomArray.
    atom_array_for_parse = preserve_explicit_nonpolymer_identity(
        atom_array_orig
    )
    # HACK: Set empty bond graph:
    if atom_array_for_parse.bonds is None:
        atom_array_for_parse.bonds = BondList(atom_array_for_parse.array_length())

    # Temporary spoof of chain IDs to ensure duplicates aren't dropped:
    result_dict = parse_atom_array(
        atom_array_for_parse,
        remove_ccds=[],
        fix_arginines=False,
